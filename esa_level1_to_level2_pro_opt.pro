...
; We calculate the inverse of C_reg using the SVD components of C_reg
    ...
    w0=REGSVD(C_inv,V,d0,s,0.d0) ; unregularised solution
...
; I. Sandberg
 ...
 IF iy eq ix THEN   C_reg[ix,iy]= -2.d0 + ksi ELSE $
...
if particles eq 'protons' then begin
      ...
      FOR ipmax= ipmin,nx_pp-1   DO BEGIN
         ...
         FOR i=0,nx-1 DO GF_I_T[i,*]=GF_I_T[i,*]*x_norma[i]
         FOR j=0,14 DO GF_I_T[*,j]=GF_I_T[*,j]/b_norm[j]
  ...
  ;RESCALING AND ROTATIONS
         ...
         FOR i=0,nx-1 DO GF_I_Tn[i,*]=GF_I_T[i,*]*x_norma[i]
...
IF n_nans eq 1 THEN BEGIN
   dummy=FINTERPOL(f1[arrv],E[arrv],[E[arr],E[arr]],/power_law_interp,/quiet)
   f1[arr]=dummy[0]
...
IF i_srem_unit eq 0 THEN restore,'/home/sandberg/data/SREM_DATA/SREM/SREM_GF_GIOVEB.idat' ELSE $
IF i_srem_unit eq 1 THEN restore,'/home/sandberg/data/SREM_DATA/SREM/SREM_GF_PROBA1.idat' ELSE $
IF i_srem_unit eq 2 THEN restore,'/home/omiros/Documents/projects/sremdc/esa/data/SREM_DATA/SREM/SREM_GF_INTEGRAL.idat' ELSE $
IF i_srem_unit eq 3 THEN restore,'/home/sandberg/data/SREM_DATA/SREM/SREM_GF_ROSETTA.idat' ELSE $
IF i_srem_unit eq 4 THEN restore,'/home/sandberg/data/SREM_DATA/SREM/SREM_GF_HERSCHEL.idat' ELSE $
IF i_srem_unit eq 5 THEN restore,'/home/sandberg/data/SREM_DATA/SREM/SREM_GF_PLANCK.idat' ELSE print, "no i_srem_unit defined!!!"
...
Eqmin=5.d0 & Eqmax=200.d0 & nx_q=10
...
;------------------------------------------------------------------;
FUNCTION extract_counts, file_o
...
function ESA_LEVEL1_to_LEVEL2, dir_pacc, file_pacc

; This function receives as input the name of the cdf file
; with SREM count-rates and provides as output SREM SVD fuxes

;-------------------------------------------------;
;---Input LEVEL 1: L1_cdf_file_name (PACC file)---;
;-------------------------------------------------;

file_o=dir_pacc+file_pacc

I=0
IF (((I = STRPOS(file_o, 'SREMGIOVEB_'))) NE -1) THEN BEGIN
sat='GIOVEB' & i_srem_unit=0 & srem_unit='SREMGIOVEB'
ENDIF

IF (((I = STRPOS(file_o, 'SREMPROBA1_'))) NE -1) THEN BEGIN
sat='PROBA1' & i_srem_unit=1 & srem_unit='SREMPROBA1'
ENDIF

IF (((I = STRPOS(file_o, 'IREM_'))) NE -1) THEN BEGIN
sat='INTEGRAL' & i_srem_unit=2 & srem_unit='IREM'
ENDIF

IF (((I = STRPOS(file_o, 'SREMROSETTA_'))) NE -1) THEN BEGIN
sat='ROSETTA' & i_srem_unit=3 & srem_unit='SREMRosetta'
ENDIF

IF (((I = STRPOS(file_o, 'SREMHERSCHEL_'))) NE -1) THEN BEGIN
sat='HERSCHEL' & i_srem_unit=4 & srem_unit='SREMHerschel'
ENDIF

IF (((I = STRPOS(file_o, 'SREMPLANCK_'))) NE -1) THEN BEGIN
sat='PLANCK' & i_srem_unit=5 & srem_unit='SREMPlanck'
ENDIF

PRINT, "SATELLITE=",sat
PRINT, "SREM unit=",srem_unit

;--------------------------------------------------------------------;
;---------Extract COUNTS from l1_cdf_file_name (PACC file)-----------;
;--------------------------------------------------------------------;
cnts=extract_counts(file_o)
help,cnts

;--------------------------------------------------------------------------------------------;
;-----Calculate the FLUXES with the SVD method - this is the CORE of the whole process-------;
;--------------------------------------------------------------------------------------------;
SREM_FLUX_PRODUCT_our=COUNTS_TO_FLUXES(cnts, i_srem_unit)
help,SREM_FLUX_PRODUCT_our
return,SREM_FLUX_PRODUCT_our
end
...
pro esa_level1_to_level2_pro_opt
; EXAMPLE
dirname='/home/omiros/Documents/projects/sremdc/esa/input/'

;pacc_file1='IREM_PACC_20050117_ESA_L1_P01.cdf'
pacc_file1='IREM_PACC_20050218_ESA_L1_V01.cdf'
;pacc_file3='.......cdf'

SREM_FLUX_PRODUCT_our=ESA_LEVEL1_to_LEVEL2(dirname,pacc_file1)

; contents
;SREM_FLUX_PRODUCT_our=create_struct('FPDO',FPDO,'FEDO',FEDO,'FPDO_ENERGY',FPDO_ENERGY,'FEDO_ENERGY',FEDO_ENERGY,'FPDO_ENERGY_bou',FPDO_ENERGY_bou,'FEDO_ENERGY_bou',FEDO_ENERGY_bou,'PARTICLE_INDEX',PARTICLE_INDEX,'GFP',GFP,'GFE',GFE)


;------------------------------------------------------------------;
;--------Save the extracted variables from FSVD to SAV files-------;
;------------------------------------------------------------------;
SAVE, SREM_FLUX_PRODUCT_our.FPDO, SREM_FLUX_PRODUCT_our.FEDO, SREM_FLUX_PRODUCT_our.FPDO_ENERGY, SREM_FLUX_PRODUCT_our.FEDO_ENERGY, SREM_FLUX_PRODUCT_our.FPDO_ENERGY_BOU, SREM_FLUX_PRODUCT_our.FEDO_ENERGY_BOU, SREM_FLUX_PRODUCT_our.PARTICLE_INDEX, SREM_FLUX_PRODUCT_our.GFP, SREM_FLUX_PRODUCT_our.GFE, /VARIABLES, FILENAME='/home/omiros/Documents/projects/sremdc/esa/output/FSVD_b.sav'
